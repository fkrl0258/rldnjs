...
public class OracleMessageDao extends MessageDao{
	...
	public int insert(Connection conn, Message message) throws SQLException{
		...
		try {
			pstmt = conn.prepareStatement("inset int kw_guestbook_message "
					+ " (message_id, guest_name, password, message) "
					+ " values (kwmessage_id_seq,NEXTVAL, ?, ?, ?)");
	...
	public List<Message> selectList(Connection conn, int firstRow, int endRow) throws SQLException{
		...
		try {
			pstmt = conn.prepareStatement("select message_id, guest_name, password, message from ("
					...
					+ "			select * from kw_guestbook_message m order by m.message_id desc "
					+ "		) where roenum <= ?"
					+ ") where rnum >= ?");
